...
app.get('/articles', function(request, response) {
  ...
  client.query(`SELECT article_id, title, category, "publishedOn", body, authors.author AS author, authors.authorUrl AS "authorUrl"
    FROM articles
    INNER JOIN authors
...
app.post('/articles', function(request, response) {
  ...
  client.query(
    ...
    [request.author, request.authorUrl]
  )
  .then(function() {
    ...
    client.query(
      ...
      FROM authors
      WHERE author=$5;`,
      [request.title, request.category, request.publishedOn, request.body, request.author]
    )
  })
  .then(function() {
